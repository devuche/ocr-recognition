...
from middleware import ocr, docformat, getdocument
...
def upload_file():
    ...
    try:
        ...
        content = ocr.analyze_read(new_filename)
        print(content)
...
@views.route('/document-query/<int:document_id>', methods=['POST'])
@swag_from({
    'tags': ['Chatbot'],
    'description': 'Query a specific document uploaded by the user using a bot',
    'parameters': [
        {
            'name': 'Content-Type',
            'in': 'header',
            ...
            if user_input:
                if conversation_id:
                    ...
                    if conversation_item:
                        ...
                        response = docformat.ai_search(user_input, conversation)
                        docformat.set_conversation(response[1], conversation_id)
                    return jsonify({"response": response[0]}), 200
                elif conversation_id is None:
                    conversation_item = docformat.get_conversation(document.content)
                    conversation = conversation_item[0]
                    conversation_id = conversation_item[1]
                    response = docformat.ai_search(user_input, conversation)
                    docformat.new_conversation(email, username, response[1], conversation_id, document_id)
...
def get_query(conversation_id):
    try:
        ...
        conversation = Conversation.query.filter_by(conversation_id=conversation_id, user_id=userEmail).first()
        if conversation:
            return jsonify({"document_id": conversation.document_id,
